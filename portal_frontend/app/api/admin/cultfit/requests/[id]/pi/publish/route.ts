...
import { publishPI, LeadNotFoundError, PIWorkflowError, type Authz } from '@/lib/odoo-server';
...
export async function POST(
  ...
  try {
    ...
    const snapshot = await publishPI(requestId, soId, authz, user.email);
    ...
    console.error('[publish PI]', e instanceof Error ? e.message : e);
    return NextResponse.json({ detail: 'Could not publish PI. Please try again.' }, { status: 503 });
  }
}
